...
router.get("/", async (req, res) => {
  try {
    ...
    // Renombrar campos a mayúsculas para la vista (roles.ejs espera ID_ROL, ROL, etc.)
    const rolesUpper = roles.map(r => ({
      ID_ROL: r.id_rol,
      ROL: r.rol,
    ...
    const objetosUpper = objetos.map(o => ({
    ...
    const permisosUpper = permisos.map(p => ({
      ID_PERMISO: p.id_permiso,
      ID_ROL: p.id_rol,
      ID_OBJETO: p.id_objeto,
      PERMISO_CONSULTA: p.permiso_consulta,
      OBJETO: p.objeto
    }));
    
    console.log('📋 Roles cargados:', rolesUpper.length);
    console.log('📋 Objetos cargados:', objetosUpper.length);
    console.log('📋 Permisos cargados:', permisosUpper.length);
    
    res.render("roles", { 
      roles: rolesUpper,
      objetos: objetosUpper,
      permisos: permisosUpper,
...
router.get("/api/roles", async (req, res) => {
  try {
    ...
    const rolesUpper = rows.map(r => ({
      ID_ROL: r.id_rol,
      ROL: r.rol,
      DESCRIPCION: r.descripcion,
      ESTADO: r.estado
    }));
    res.json({ ok: true, roles: rolesUpper });
  } catch (error) {
...
router.get("/api/objetos", async (req, res) => {
  try {
    const { rows } = await pool.query(`
      ...
      ORDER BY id_objeto
    `);
    const objetosUpper = rows.map(o => ({
      ID_OBJETO: o.id_objeto,
      OBJETO: o.objeto,
      DESCRIPCION: o.descripcion,
      TIPO_OBJETO: o.tipo_objeto
    }));
    res.json({ ok: true, objetos: objetosUpper });
  } catch (error) {
...
router.get("/api/permisos/:idRol", async (req, res) => {
  try {
    ...
    const permisosUpper = rows.map(p => ({
      ID_PERMISO: p.id_permiso,
      ID_ROL: p.id_rol,
      ID_OBJETO: p.id_objeto,
      PERMISO_CONSULTA: p.permiso_consulta,
      OBJETO: p.objeto,
      OBJETO_DESCRIPCION: p.objeto_descripcion
    }));
    
    res.json({ ok: true, permisos: permisosUpper });
  } catch (error) {
