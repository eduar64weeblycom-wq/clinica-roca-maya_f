(() => {
    ...
    function resetearEstilosBotonesModal() {
        ...
        const btnImprimir = $('btnImprimirConsulta');
        if (btnImprimir) {
        ...
        const btnCancelar = $('btnCancelarConsulta');
        if (btnCancelar) {
    ...
    function renderizarTabla() {
        ...
        if (!citasFiltradas.length) {
            ...
            `;
            const btnCrear = $('btnCrearPrimeraConsulta');
    ...
    function cerrarModalConsulta() {
        ...
        const historialContainer = $('historialRapidoPaciente');
        if (historialContainer) {
            historialContainer.style.display = 'none';
            const contenido = $('historialRapidoContenido');
    ...
    function limpiarModalConsulta() {
        ...
        // limpiarMedicamentos(); // Asumiendo que esta función existe externamente o se gestiona aparte
        const btnFinalizar = $('btnFinalizarConsulta');
        ...
        if (preBox) {
            ...
            const mensaje = $('preclinicaMensaje');
            if (mensaje) {
    ...
    function aplicarEstadoPreclinica() {
        const fields = $('preclinicaFields');
        const toggleBtn = $('togglePreclinica');
    ...
    window.togglePreclinica = function() {
        ...
        const toggleBtn = $('togglePreclinica');
        if (!fields || !toggleBtn) {
            console.warn('Preclínica no disponible');
            return;
        }
    ...
    async function cargarTodosLosDatosDeCita(idCita, idPaciente, idConsultaExistente) {
        ...
        try {
            ...
            const [consultaData, preclinicaData, historialData] = await Promise.all(promises);

            if (consultaData && consultaData.success && consultaData.consulta) {
                // Llenar datos de consulta si procede
            ...
            if (err.name !== 'AbortError') {
                console.error("Error al cargar los datos completos de la cita:", err);
            }
        } finally {
            cargandoDatos = false;
        }
