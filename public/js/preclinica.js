...
// ============================================================
// EXPONER API PÚBLICA PARA CONSULTA MÉDICA
// ============================================================

window.Preclinica = {
    // Carga los datos de preclínica para una cita y los renderiza en el contenedor
    renderizarCampos: async function(idCita, containerSelector = '#preclinicaFields') {
        ...
        try {
            ...
            if (data.success && data.preclinica) {
                ...
                set("frecuenciaCardiaca", p.FRECUENCIA_CARDIACA);
                set("frecuenciaRespiratoria", p.FRECUENCIA_RESPIRATORIA);
                set("saturacionOxigeno", p.SATURACION_OXIGENO);
                set("peso", p.PESO);
                set("talla", p.TALLA);
                set("glucosa", p.GLUCOSA);
                set("perimetroAbdominal", p.PERIMETRO_ABDOMINAL);
                set("observaciones", p.OBSERVACIONES);
                ...
                const tallaNum = parseFloat(p.TALLA) || 0;
                if (pesoNum && tallaNum) {
    ...
    // Limpia los campos del formulario de preclínica dentro de un contenedor
    limpiarCampos: function(container) {
        const inputs = container.querySelectorAll('input, textarea, select');
        inputs.forEach(input => {
            if (input.id === 'estadoGeneral') {
                input.value = 'BUENO';
            } else if (input.id !== 'imc') {
                input.value = '';
            }
        });
        const imcField = document.getElementById('imc');
    ...
    obtenerDatosFormulario: function(container) {
        const get = (id) => {
            const el = document.getElementById(id);
        ...
        return {
            temperatura: parseNum(get('temperatura')),
            presionSistolica: parseNum(get('presionSistolica')),
            presionDiastolica: parseNum(get('presionDiastolica')),
            frecuenciaCardiaca: parseNum(get('frecuenciaCardiaca')),
            frecuenciaRespiratoria: parseNum(get('frecuenciaRespiratoria')),
            saturacionOxigeno: parseNum(get('saturacionOxigeno')),
            peso: parseNum(get('peso')),
            talla: parseNum(get('talla')),
            glucosa: parseNum(get('glucosa')),
            perimetroAbdominal: parseNum(get('perimetroAbdominal')),
            observaciones: get('observaciones') || null,
            estadoGeneral: get('estadoGeneral') || 'BUENO'
        };
    },

    // Obtiene la preclínica existente para una cita
    obtenerPreclinicaPorCita: async function(idCita) {
        try {
            const res = await fetch(`/preclinica/por-cita/${idCita}`, {
                credentials: 'same-origin'
            });
            ...
            console.warn('Error obteniendo preclínica por cita:', err);
            return null;
        }
    },

    // Guarda o actualiza una preclínica
    guardarPreclinica: async function(payload) {
        // payload debe contener: idCita, y todos los campos clínicos
        // También puede incluir idPreclinica si es actualización
        const isUpdate = payload.idPreclinica && payload.idPreclinica > 0;
        const url = isUpdate ? '/preclinica/actualizar' : '/preclinica/nueva';

        // Asegurarse de que enviarAConsulta esté definido
        if (payload.enviarAConsulta === undefined) {
            payload.enviarAConsulta = true;
        }
    ...
    // Función para alternar visibilidad de la preclínica (ya implementada globalmente)
    // Esta función ya está en window.togglePreclinica, pero la incluimos por si acaso.
...
console.log('✅ API de Preclinica expuesta globalmente.');